...
- [ ] **Combine usage despite "use async/await" rule.** `Pluralizer`, `Timer`, `Reachability`, `CommunalFetcher`, `DeSync`, `ObservableValue`, `SignificantTimeChangeObserver`, `InterfaceOrientedView`, `PublisherView`, `SceneStateObserver`, `Keychain`, `onTimer`, `NotificationObserver`, `Debouncer`.
    ...
    - **Open**: `Reachability`, `SceneStateObserver`, `Keychain`. Each needs a focused look (`Reachability` already had its memory-leak / multi-waiter bug fixed in fab966b but still uses `NWPathMonitor` with closures + `objectWillChange.send`; `SceneStateObserver` retain-cycle was fixed but the `[AnyCancellable]` driver is still Combine; `Keychain` mixes Combine for some of its async surface). Plus the `Subscribers.Sink` leak in `Publishers.swift` from Tier A's #10.
