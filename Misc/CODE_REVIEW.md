...
- [ ] **Deprecated APIs in active use**: `NavigationLink(isActive:)` *[partially: removed from `BoundNavigationLink` via fab966b file delete; other call sites remain]*, `NavigationView`, single-arg `onChange`, `UIScreen.main`, `UIApplication.windows`, `UIGraphicsBeginImageContextWithOptions`, `Process.launchPath`/`launch()`, `kIOMasterPortDefault`, `.autocapitalization` *[fixed: `addTextContentType` now uses `.textInputAutocapitalization` on iOS 15+, falls back on older]*, `AnimatableModifier` *[fixed: `AnimationCompletionObserverModifier` now conforms to `ViewModifier, Animatable` separately]*. *`NavigationView`/`NavigationStack` is a behavior change. `kIOMasterPortDefault` → `kIOMainPortDefault` requires macOS 12+ availability check (Suite targets macOS 10.15); the values are identical at runtime so the deprecation is cosmetic. `UIGraphicsBeginImageContextWithOptions` → `UIGraphicsImageRenderer` is mechanical but spans 5 call sites in `UIImage.swift`.*
